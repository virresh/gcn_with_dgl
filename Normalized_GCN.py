#### Running Normalized GCN with karate club dataset
import dgl
import torch
...
# Use built-in functions from DGL for message
# and reduce functions
def gcn_msg(edges):
    # normalize source embedding with sqrt(source degree) and
    # send to destination vertex
    return {
        'm': edges.src['deg_root'][:, None] * edges.src['h']
    }


def gcn_reduce(nodes):
    # normalize neighbor embeddings with sqrt(self degree) and
    # store sum in current embedding
    deg_sq = nodes.data['deg_root'].reshape(-1)[0]
    # can use any element from deg_sq since all elements will be same
    return {'h': deg_sq * torch.sum(nodes.mailbox['m'], dim=1)}
...
class GCNLayer(nn.Module):
    ...
    def forward(self, g, feature):
        ...
        with g.local_scope():
            g.ndata['h'] = feature
            # pass along 1/sqrt(i) for all nodes, to be used during
            # reduction and message computation
            g.ndata['deg_root'] = 1/torch.sqrt(
                (g.in_degrees(g.nodes())).type(torch.FloatTensor))
